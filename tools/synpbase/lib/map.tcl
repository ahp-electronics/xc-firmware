...
# $Header: //synplicity/maplat2018q2p1/misc/map.tcl#1 $
...
proc synplify_set_sdc_where { args } {
        global fdc_line
        global fdc_file
	# file name that this proc was called from
	if { [catch {info frame} i ] } {
		...
		for { incr i -1 } { $i > 0 } { incr i -1 } {
			...
			if { $file != $here } {
				# report file lowest on stack which is file
				# that this proc was called from 
...
proc set_rtl_ff_names { args } {
	...
	if { $_lib_=="SETCONST"} {
		...
		catch [eval synplify_set_rtl_ff_names [lrange [info level 0] 1 end] ]
		addConstraint2Database [lindex [info level 0] 0] [lrange [info level 0] 1 end]
	}
}
proc bus_dimension_separator_style { args } {
    global _lib_
    if {$_lib_ == "SYNPLIFY"} {
		synplify_bus_dimension_separator_style [lrange [info level 0] 1 end]
		addConstraint2Database [lindex [info level 0] 0] [lrange [info level 0] 1 end]
    } elseif {$_lib_ == "SETCONST"} {
        syn_wrong_sdc_format
    }
...
proc bus_naming_style { args } {
    global _lib_
    if {$_lib_ == "SYNPLIFY"} {
		synplify_bus_naming_style [lrange [info level 0] 1 end]
		addConstraint2Database [lindex [info level 0] 0] [lrange [info level 0] 1 end]
    } elseif {$_lib_ == "SETCONST"} {
        syn_wrong_sdc_format
    }
...
proc set_disable_timing { args } {
	...
	warning {@W: set_disable_timing not supported.}
