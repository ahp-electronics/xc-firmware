...
if {[product_type] eq "protocompiler" || [product_type] eq "protocompiler_dx"} {
...
set options(proto_correlate,-scf_mode)                 [add_option "-scf_mode		excl	{-xdc_mode,-sdc_verif}	reqs	{-pr_dir}"]
set options(proto_correlate,-xdc_mode)                 [add_option "-xdc_mode		excl    {-scf_mode}		reqs	{-pr_dir}"]
set options(proto_correlate,-xdc_place)                [add_option "-xdc_place		reqs	{-pr_dir,-xdc_mode}"]
set options(proto_correlate,-pr_dir)                   [add_option "-pr_dir		plural	{1}	<value>	<strg>"]
...
set options(proto_correlate,-ta_index)                 [add_option "-ta_index		plural	{1}	<value>	<list>"]
set options(proto_correlate,-qii_index)                [add_option "-qii_index		plural	{1}	<value>	<list>"]
set options(proto_correlate,-trce_index)               [add_option "-trce_index		plural	{1}	<value>	<list>"]
set options(proto_correlate,-paths_per)                [add_option "-paths_per		plural	{1}	<value>	<int>"]
...
set options(proto_correlate,-sdc_verif)                [add_option "-sdc_verif		plural	{1}"]
...
set options(proto_correlate,-load_sta)                 [add_option "-load_sta		plural	{1}"]
set options(proto_correlate,-default_sta)              [add_option "-default_sta	plural	{1}"]
set options(proto_correlate,-back_anno)                [add_option "-back_anno		plural	{1}"]
...
############################################
####       get_ports      ##############
set options(get_ports,-hierarchical)                  [add_option "-hier.*		excl	{-of_objects}	<help>"]
...
proc get_opt_val {cmd opt} {
  variable options
  if {[catch {set val [lindex $options($cmd,$opt) 4]}] || [lindex $options($cmd,$opt) 4] eq "" || [lindex $options($cmd,$opt) 3] eq "<help>"} {
    return 0
  } else {
    set i 5
...
#### -back_anno option
    if {[check_opt_is proto_correlate {-back_anno} [lindex $args $x]]} {
      if {$state eq "map"} {
        set back_anno "ba"
      } else {
        set back_anno "slta"
...
#### Setup needed files   ####
  set top_proto $top
  if {!$check_hstdm} {
    if {[catch {glob -directory . ${top}_ctd.txt} CTD] || [file mtime ${top}.srm] > [file mtime ${top}_ctd.txt]} {
...
#### gold synth --> synth correlate
    ...
    regsub {proto_correlate} $corr_cmd "corr_synth_gui" corr_cmd
    if {$result_gold eq ""} {
      set cmd "$corr_cmd -impl_name $impl_name -impl_result $impl_result -result_gold $impl_result"
    } else {
...
#### -write_fdc option
  ...
  if {$prod eq "legacy"} {
    ...
    if {$file_xdc} {
      if {![regexp {^\/} $xdc_file]} {
        set xdc_file $start_dir/$xdc_file
      }
    }
  ...
  if {$do_all || $r2t} {
    ...
    if {$prod eq "legacy"} {
      ...
      if {![catch {report timing -generate -from $rx_clocks -to $tx_clocks  -out $top -write_ctd -quiet}]} {
        ...
        eval $cmd_strg
        incr hstdm_found
      } elseif {![catch {report timing -generate -from [get_cells {cpm_rcv*.data_out[*]}] -to [get_cells {cpm_snd*.cpm_data}]}]} {
          set cmd_strg "proto_correlate -effort low -paths_per 100000 -sdc_verif -load_sta -hstdm_report $out_file"
          eval $cmd_strg
          incr hstdm_found
      } else {
...
proc goto_work_dir {} {cd [database query -path]; cd ..; pwd}
