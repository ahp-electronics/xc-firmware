...
proc report_hstdm_qor {args} {
    ...
    foreach pblock "[get_pblocks -quiet]" {
        ...
        while {$end>=0} {
            ...
            if {[regexp "HHH *Slice *HHH *([0-9]+) HHH *([0-9]+) HHH *([0-9]+) HHH *([0-9]+) HHH *([0-9]+) HHH *(.*) HHH" $line dummy assign external used loced available util]} {
                lappend pblock_util "$pblock $assign $external $used $loced $available $util"
                break
            }
            # it is CLB for UltraScale
            if {[regexp "HHH *CLB *HHH *([0-9]+) HHH *([0-9]+) HHH *([0-9]+) HHH *([0-9]+) HHH *([0-9]+) HHH *(.*) HHH" $line dummy assign external used loced available util]} {
                lappend pblock_util "$pblock $assign $external $used $loced $available $util"
                break
            }
            set start [expr {$end+1}]
    ...
    if {$pblock_util!=""} {
        ...
        set pblock_table_header  "{pblock name} {assign} {external} {used} {loced} {available} {utilization}"
...
proc report_hstdm_missing_constraints {args} {
    ...
    foreach hstdm_pin [lsort $all_hstdm_pins] {
        ...
        if {$start_point_is_user} {
            set hstdm_point [all_fanout -endpoints_only -flat [get_pins $hstdm_pin] -quiet]
        } else {
            # if we do not add -only_cells option, the start point will be the clock pin
            set hstdm_point [all_fanin -startpoints_only -only_cells -flat [get_pins $hstdm_pin] -quiet]
        }
        ...
        if {$start_point_is_user} {
            set hstdm_point_string "-through \[get_pins {$hstdm_pin}\] -to \[get_pins {$hstdm_point}\]"
        } else {
    ...
    proc hstdm_get_user_clock_string {timing_path user_clk_prop from_or_to_user} {
        ...
        set clk_period [get_property PERIOD [eval $get_clocks_str]]
        if {$clk_period==0 || $clk_period==""} {
            set edge_string ""
        } else {
            set user_clk_edge_prop "${user_clk_prop}_EDGE"
            set user_clk_edge [get_property $user_clk_edge_prop $timing_path]
            set clk_waveform [get_property WAVEFORM [eval $get_clocks_str]]
            set clk_rising [lindex $clk_waveform 0]
            set is_rising_edge [expr {abs(fmod(($user_clk_edge-$clk_rising), $clk_period))<=0.001}]
            if {$is_rising_edge} {
    ...
    if {($found_hstdm_issues>0) || ($error_list!="")} {
        ...
        lappend header "# Copyright 2017 Synopsys, Inc."
