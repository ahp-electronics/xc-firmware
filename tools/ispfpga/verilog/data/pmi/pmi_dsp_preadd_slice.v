...
---------------------------------------------------------------------------------
*/
// fpga\verilog\pkg\versclibs\data\pmi\pmi_dsp_preadd_slice.v 1.5 30-SEP-2011 08:05:44 PPUTREVU
//
...
//pragma translate_off
   ...
   wire 		      scuba_vhi;
   wire 		      scuba_vlo;
   wire [17:0] tSRIA, tM0A, tSRIB, tM1A, tSROA;
   wire [17:0] tM0B, tM1B;
   wire [53:0] C;
   wire [17:0] mSRIA, mSROA;
   wire [35:0] mult_out_p0, mult_out_p1, tmult_out_p0, tmult_out_p1;
      
   wire        m0SOURCEA, m1SOURCEA;
   
   
   
   // Left align M0A, M0B, M1A, M1B
   wire [17:0] sM0A = (pmi_pre0_sourcea_mode == "ZERO") ? 18'b0 :
        (pmi_data_m0a_width == 18) ? M0A : {M0A, {(18-pmi_data_m0a_width){1'b0}}};
   
   wire [17:0] sM0B = (pmi_pre0_sourceb_mode == "ZERO") ? 18'b0 : 
        (pmi_data_m0b_width == 18) ? M0B : {M0B, {(18-pmi_data_m0b_width){1'b0}}};

   wire [17:0] sM1A = (pmi_pre1_sourcea_mode == "ZERO") ? 18'b0 : 
        (pmi_data_m1a_width == 18) ? M1A : {M1A, {(18-pmi_data_m1a_width){1'b0}}};

   wire [17:0] sM1B = (pmi_pre1_sourceb_mode == "ZERO") ? 18'b0 :
        (pmi_data_m1b_width == 18) ? M1B : {M1B, {(18-pmi_data_m1b_width){1'b0}}};

   wire [17:0] sM0C = (pmi_data_m0c_width == 18) ? M0C :
       {M0C, {(18-pmi_data_m0c_width){1'b0}}};

   wire [17:0] sM1C = (pmi_data_m1c_width == 18) ? M1C :
       {M1C, {(18-pmi_data_m1c_width){1'b0}}};
   ...
   assign      tM0B = (pmi_data_m0b_width == 18) ? M0B : {M0B, {(18-pmi_data_m0b_width){1'b0}}};
   assign      tM1B = (pmi_data_m1b_width == 18) ? M1B : {M1B, {(18-pmi_data_m1b_width){1'b0}}};

   assign      SROA = (pmi_m1_sourcea_mode == "OPA1") ? tSROA : mSROA;
   

   assign      m0SOURCEA = (pmi_m0_sourcea_mode == "A_SHIFT") ? SOURCEM0A : 1'b0;
   assign      m1SOURCEA = (pmi_m1_sourcea_mode == "A_SHIFT") ? SOURCEM1A : 1'b0;
   
   
   localparam  pre0_sourceb_mode = (pmi_pre0_sourceb_mode == "ZERO") 
                   ? "PARALLEL" : pmi_pre0_sourceb_mode;
   localparam  pre0_sourcea_mode = (pmi_pre0_sourcea_mode == "ZERO") 
                   ? "PARALLEL" : pmi_pre0_sourcea_mode;
   localparam  pre1_sourceb_mode = (pmi_pre1_sourceb_mode == "ZERO") 
                   ? "PARALLEL" : pmi_pre1_sourceb_mode;
   localparam  pre1_sourcea_mode = (pmi_pre1_sourcea_mode == "ZERO") 
                   ? "PARALLEL" : pmi_pre1_sourcea_mode;


   generate

   if ((pmi_m0_sourcea_mode == "OPA0") &&
       (pmi_m1_sourcea_mode == "OPA1")) begin : pradd
   
      defparam dsp_preadd_0.SYMMETRY_MODE = pmi_pre0_symmetry_mode ;
      defparam dsp_preadd_0.FB_MUX = "DISABLED" ; // ALWAYS DISABLED
      defparam dsp_preadd_0.SOURCEB_MODE = pre0_sourceb_mode ;
      defparam dsp_preadd_0.SOURCEA_MODE = pre0_sourcea_mode ;
      ...
      defparam dsp_preadd_1.SOURCEB_MODE = pre1_sourceb_mode ; 
      defparam dsp_preadd_1.SOURCEA_MODE =  pre1_sourcea_mode;
      defparam dsp_preadd_1.CAS_MATCH_REG = (pmi_cascade_match_reg == "on") ? "TRUE" : "FALSE";
      defparam dsp_preadd_1.RESETMODE = "ASYNC" ;
      defparam dsp_preadd_1.GSR = "ENABLED" ;
      defparam dsp_preadd_1.REG_OPPRE_RST = pmi_reg_oppre1_rst; 
      defparam dsp_preadd_1.REG_OPPRE_CE = pmi_reg_oppre1_ce; 
      defparam dsp_preadd_1.REG_OPPRE_CLK = pmi_reg_oppre1_clk ; 
      ...
      PRADD18A dsp_preadd_1 (.PA17(sM1A[17]), .PA16(sM1A[16]), .PA15(sM1A[15]), .PA14(sM1A[14]), 
        ...
        .SROA17(tSROA[17]), .SROA16(tSROA[16]), .SROA15(tSROA[15]), .SROA14(tSROA[14]), 
        .SROA13(tSROA[13]), .SROA12(tSROA[12]), .SROA11(tSROA[11]), .SROA10(tSROA[10]), .SROA9(tSROA[9]), 
        .SROA8(tSROA[8]), .SROA7(tSROA[7]), .SROA6(tSROA[6]), .SROA5(tSROA[5]), .SROA4(tSROA[4]), 
        .SROA3(tSROA[3]), .SROA2(tSROA[2]), .SROA1(tSROA[1]), .SROA0(tSROA[0]),
   ...
   generate
      if ((pmi_m0_sourcea_mode != "OPA0") ||
	  (pmi_m1_sourcea_mode != "OPA1")) begin : pradd_bypass

	 assign tM0A = (pmi_data_m0a_width == 18) ? M0A : {M0A, {(18-pmi_data_m0a_width){1'b0}}};
	 assign tM1A = (pmi_data_m1a_width == 18) ? M1A : {M1A, {(18-pmi_data_m1a_width){1'b0}}};
   ...
   endgenerate
    ...
    MULT18X18D dsp_mult_0 (.A17(tM0A[17]), .A16(tM0A[16]), .A15(tM0A[15]), .A14(tM0A[14]), 
        ...
        .A2(tM0A[2]), .A1(tM0A[1]), .A0(tM0A[0]), .B17(tM0B[17]), .B16(tM0B[16]), 
        .B15(tM0B[15]), .B14(tM0B[14]), .B13(tM0B[13]), .B12(tM0B[12]), .B11(tM0B[11]), 
        .B10(tM0B[10]), .B9(tM0B[9]), .B8(tM0B[8]), .B7(tM0B[7]), .B6(tM0B[6]), .B5(tM0B[5]), 
        .B4(tM0B[4]), .B3(tM0B[3]), .B2(tM0B[2]), .B1(tM0B[1]), .B0(tM0B[0]), 
        .C17(sM0C[17]), .C16(sM0C[16]), .C15(sM0C[15]), .C14(sM0C[14]), .C13(sM0C[13]), 
        .C12(sM0C[12]), .C11(sM0C[11]), .C10(sM0C[10]), .C9(sM0C[9]), .C8(sM0C[8]), .C7(sM0C[7]), 
        .C6(sM0C[6]), .C5(sM0C[5]), .C4(sM0C[4]), .C3(sM0C[3]), .C2(sM0C[2]), 
        .C1(sM0C[1]), .C0(sM0C[0]),
        .SIGNEDA(SIGNM0A), .SIGNEDB(SIGNM0B), .SOURCEA(m0SOURCEA), .SOURCEB(1'b0), 
        .CE0(CE[0]), .CE1(CE[1]), .CE2(CE[2]), .CE3(CE[3]),
        .CLK0(CLK[0]), .CLK1(CLK[1]), .CLK2(CLK[2]), .CLK3(CLK[3]),
        .RST0(RST[0]), .RST1(RST[1]), .RST2(RST[2]), .RST3(RST[3]),
        .SRIA17(SRIA[17]), .SRIA16(SRIA[16]), .SRIA15(SRIA[15]), .SRIA14(SRIA[14]), 
        ...
        .SRIA8(SRIA[8]), .SRIA7(SRIA[7]), .SRIA6(SRIA[6]), .SRIA5(SRIA[5]), .SRIA4(SRIA[4]), 
        .SRIA3(SRIA[3]), .SRIA2(SRIA[2]), .SRIA1(SRIA[1]), .SRIA0(SRIA[0]),			   
        .SRIB17(1'b0), .SRIB16(1'b0), .SRIB15(1'b0), 
        ...
        .SRIB2(1'b0), .SRIB1(1'b0), .SRIB0(1'b0), 
	.SROA17(mSRIA[17]), .SROA16(mSRIA[16]), .SROA15(mSRIA[15]), .SROA14(mSRIA[14]), 
        .SROA13(mSRIA[13]), .SROA12(mSRIA[12]), .SROA11(mSRIA[11]), .SROA10(mSRIA[10]), .SROA9(mSRIA[9]), 
        .SROA8(mSRIA[8]), .SROA7(mSRIA[7]), .SROA6(mSRIA[6]), .SROA5(mSRIA[5]), .SROA4(mSRIA[4]), 
        .SROA3(mSRIA[3]), .SROA2(mSRIA[2]), .SROA1(mSRIA[1]), .SROA0(mSRIA[0]),
	.SROB17(), .SROB16(), .SROB15(), 
        ...
        .ROC1(), .ROC0(),
        .P35(mult_out_p0[35]),.P34(mult_out_p0[34]),.P33(mult_out_p0[33]),
        .P32(mult_out_p0[32]),.P31(mult_out_p0[31]),.P30(mult_out_p0[30]),
        .P29(mult_out_p0[29]),.P28(mult_out_p0[28]),.P27(mult_out_p0[27]),
        .P26(mult_out_p0[26]),.P25(mult_out_p0[25]),.P24(mult_out_p0[24]),
        .P23(mult_out_p0[23]),.P22(mult_out_p0[22]),.P21(mult_out_p0[21]),
        .P20(mult_out_p0[20]),.P19(mult_out_p0[19]),.P18(mult_out_p0[18]),
        .P17(mult_out_p0[17]),.P16(mult_out_p0[16]),.P15(mult_out_p0[15]),
        .P14(mult_out_p0[14]),.P13(mult_out_p0[13]),.P12(mult_out_p0[12]),
        .P11(mult_out_p0[11]),.P10(mult_out_p0[10]),.P9(mult_out_p0[9]),
        .P8(mult_out_p0[8]),.P7(mult_out_p0[7]),.P6(mult_out_p0[6]),
        .P5(mult_out_p0[5]),.P4(mult_out_p0[4]),.P3(mult_out_p0[3]),
        .P2(mult_out_p0[2]),.P1(mult_out_p0[1]),.P0(mult_out_p0[0]),
    ...
    defparam dsp_mult_1.CAS_MATCH_REG = (pmi_m1_sourcea_mode == "OPA1") ? "FALSE" : (pmi_cascade_match_reg == "on" ) ? "TRUE" : "FALSE" ; // ONLY if PREADD not used
    ...
    MULT18X18D dsp_mult_1 (.A17(tM1A[17]), .A16(tM1A [16]), .A15(tM1A[15]), .A14(tM1A[14]), 
        ...
        .A2(tM1A[2]), .A1(tM1A[1]), .A0(tM1A[0]), .B17(tM1B[17]), .B16(tM1B[16]), 
        .B15(tM1B[15]), .B14(tM1B[14]), .B13(tM1B[13]), .B12(tM1B[12]), .B11(tM1B[11]), 
        .B10(tM1B[10]), .B9(tM1B[9]), .B8(tM1B[8]), .B7(tM1B[7]), .B6(tM1B[6]), .B5(tM1B[5]), 
        .B4(tM1B[4]), .B3(tM1B[3]), .B2(tM1B[2]), .B1(tM1B[1]), .B0(tM1B[0]), 
        .C17(sM1C[17]), .C16(sM1C[16]), .C15(sM1C[15]), .C14(sM1C[14]), .C13(sM1C[13]), 
        .C12(sM1C[12]), .C11(sM1C[11]), .C10(sM1C[10]), .C9(sM1C[9]), .C8(sM1C[8]), .C7(sM1C[7]), 
        .C6(sM1C[6]), .C5(sM1C[5]), .C4(sM1C[4]), .C3(sM1C[3]), .C2(sM1C[2]), 
        .C1(sM1C[1]), .C0(sM1C[0]),
        .SIGNEDA(SIGNM1A), .SIGNEDB(SIGNM1B), .SOURCEA(m1SOURCEA), .SOURCEB(1'b0), 
        .CE0(CE[0]), .CE1(CE[1]), .CE2(CE[2]), .CE3(CE[3]),
        .CLK0(CLK[0]), .CLK1(CLK[1]), .CLK2(CLK[2]), .CLK3(CLK[3]),
        .RST0(RST[0]), .RST1(RST[1]), .RST2(RST[2]), .RST3(RST[3]),
	.SRIA17(mSRIA[17]), .SRIA16(mSRIA[16]), .SRIA15(mSRIA[15]), .SRIA14(mSRIA[14]), 
        .SRIA13(mSRIA[13]), .SRIA12(mSRIA[12]), .SRIA11(mSRIA[11]), .SRIA10(mSRIA[10]), .SRIA9(mSRIA[9]), 
        .SRIA8(mSRIA[8]), .SRIA7(mSRIA[7]), .SRIA6(mSRIA[6]), .SRIA5(mSRIA[5]), .SRIA4(mSRIA[4]), 
        .SRIA3(mSRIA[3]), .SRIA2(mSRIA[2]), .SRIA1(mSRIA[1]), .SRIA0(mSRIA[0]),		   
        .SRIB17(1'b0), .SRIB16(1'b0), .SRIB15(1'b0), 
        .SRIB14(1'b0), .SRIB13(1'b0), .SRIB12(1'b0), .SRIB11(1'b0), 
        .SRIB10(1'b0), .SRIB9(1'b0), .SRIB8(1'b0), .SRIB7(1'b0), 
        .SRIB6(1'b0), .SRIB5(1'b0), .SRIB4(1'b0), .SRIB3(1'b0), 
        .SRIB2(1'b0), .SRIB1(1'b0), .SRIB0(1'b0), 
	.SROA17(mSROA[17]), .SROA16(mSROA[16]), .SROA15(mSROA[15]), .SROA14(mSROA[14]), 
        .SROA13(mSROA[13]), .SROA12(mSROA[12]), .SROA11(mSROA[11]), .SROA10(mSROA[10]), .SROA9(mSROA[9]), 
        .SROA8(mSROA[8]), .SROA7(mSROA[7]), .SROA6(mSROA[6]), .SROA5(mSROA[5]), .SROA4(mSROA[4]), 
        .SROA3(mSROA[3]), .SROA2(mSROA[2]), .SROA1(mSROA[1]), .SROA0(mSROA[0]),
        .SROB17(), .SROB16(), .SROB15(), 
        .SROB14(), .SROB13(), .SROB12(), .SROB11(), .SROB10(), .SROB9(), 
        .SROB8(), .SROB7(), .SROB6(), .SROB5(), .SROB4(), .SROB3(), .SROB2(), 
        ...
        .ROC8(), .ROC7(), .ROC6(), .ROC5(), .ROC4(), .ROC3(), .ROC2(), 
        .ROC1(), .ROC0(),
	.P35(mult_out_p1[35]),.P34(mult_out_p1[34]),.P33(mult_out_p1[33]),
        .P32(mult_out_p1[32]),.P31(mult_out_p1[31]),.P30(mult_out_p1[30]),
        .P29(mult_out_p1[29]),.P28(mult_out_p1[28]),.P27(mult_out_p1[27]),
        .P26(mult_out_p1[26]),.P25(mult_out_p1[25]),.P24(mult_out_p1[24]),
        .P23(mult_out_p1[23]),.P22(mult_out_p1[22]),.P21(mult_out_p1[21]),
        .P20(mult_out_p1[20]),.P19(mult_out_p1[19]),.P18(mult_out_p1[18]),
        .P17(mult_out_p1[17]),.P16(mult_out_p1[16]),.P15(mult_out_p1[15]),
        .P14(mult_out_p1[14]),.P13(mult_out_p1[13]),.P12(mult_out_p1[12]),
        .P11(mult_out_p1[11]),.P10(mult_out_p1[10]),.P9(mult_out_p1[9]),
        .P8(mult_out_p1[8]),.P7(mult_out_p1[7]),.P6(mult_out_p1[6]),
        .P5(mult_out_p1[5]),.P4(mult_out_p1[4]),.P3(mult_out_p1[3]),
        .P2(mult_out_p1[2]),.P1(mult_out_p1[1]),.P0(mult_out_p1[0]),
	...
	.MA35(tmult_out_p0[35]),.MA34(tmult_out_p0[34]),.MA33(tmult_out_p0[33]),
        .MA32(tmult_out_p0[32]),.MA31(tmult_out_p0[31]),.MA30(tmult_out_p0[30]),
        .MA29(tmult_out_p0[29]),.MA28(tmult_out_p0[28]),.MA27(tmult_out_p0[27]),
        .MA26(tmult_out_p0[26]),.MA25(tmult_out_p0[25]),.MA24(tmult_out_p0[24]),
        .MA23(tmult_out_p0[23]),.MA22(tmult_out_p0[22]),.MA21(tmult_out_p0[21]),
        .MA20(tmult_out_p0[20]),.MA19(tmult_out_p0[19]),.MA18(tmult_out_p0[18]),
        .MA17(tmult_out_p0[17]),.MA16(tmult_out_p0[16]),.MA15(tmult_out_p0[15]),
        .MA14(tmult_out_p0[14]),.MA13(tmult_out_p0[13]),.MA12(tmult_out_p0[12]),
        .MA11(tmult_out_p0[11]),.MA10(tmult_out_p0[10]),.MA9(tmult_out_p0[9]),
        .MA8(tmult_out_p0[8]),.MA7(tmult_out_p0[7]),.MA6(tmult_out_p0[6]),
        .MA5(tmult_out_p0[5]),.MA4(tmult_out_p0[4]),.MA3(tmult_out_p0[3]),
        .MA2(tmult_out_p0[2]),.MA1(tmult_out_p0[1]),.MA0(tmult_out_p0[0]),
	.MB35(tmult_out_p1[35]),.MB34(tmult_out_p1[34]),.MB33(tmult_out_p1[33]),
        .MB32(tmult_out_p1[32]),.MB31(tmult_out_p1[31]),.MB30(tmult_out_p1[30]),
        .MB29(tmult_out_p1[29]),.MB28(tmult_out_p1[28]),.MB27(tmult_out_p1[27]),
        .MB26(tmult_out_p1[26]),.MB25(tmult_out_p1[25]),.MB24(tmult_out_p1[24]),
        .MB23(tmult_out_p1[23]),.MB22(tmult_out_p1[22]),.MB21(tmult_out_p1[21]),
        .MB20(tmult_out_p1[20]),.MB19(tmult_out_p1[19]),.MB18(tmult_out_p1[18]),
        .MB17(tmult_out_p1[17]),.MB16(tmult_out_p1[16]),.MB15(tmult_out_p1[15]),
        .MB14(tmult_out_p1[14]),.MB13(tmult_out_p1[13]),.MB12(tmult_out_p1[12]),
        .MB11(tmult_out_p1[11]),.MB10(tmult_out_p1[10]),.MB9(tmult_out_p1[9]),
        .MB8(tmult_out_p1[8]),.MB7(tmult_out_p1[7]),.MB6(tmult_out_p1[6]),
        .MB5(tmult_out_p1[5]),.MB4(tmult_out_p1[4]),.MB3(tmult_out_p1[3]),
        .MB2(tmult_out_p1[2]),.MB1(tmult_out_p1[1]),.MB0(tmult_out_p1[0]),		      
        ...
        .SIGNEDCIN(SIGNCIN),
        .OP10(OP[3]), .OP9(OP[2]), .OP8(OP[1]), .OP7(OP[0]),
   ...
   if ((pmi_data_m0a_width <= 9) &&
       (pmi_data_m0b_width <= 9) &&
       (pmi_data_m1a_width <= 9) &&
       (pmi_data_m1b_width <= 9)) begin : s9mode

      assign tmult_out_p0 = mult_out_signedp_0 ? {{18{mult_out_p0[35]}},mult_out_p0[35:18]} : {{18{1'b0}},mult_out_p0[35:18]};
      assign tmult_out_p1 = mult_out_signedp_1 ? {{18{mult_out_p1[35]}},mult_out_p1[35:18]} : {{18{1'b0}},mult_out_p1[35:18]};

      
   end else begin
      assign tmult_out_p0 = mult_out_p0;
      assign tmult_out_p1 = mult_out_p1;
   end
   endgenerate
