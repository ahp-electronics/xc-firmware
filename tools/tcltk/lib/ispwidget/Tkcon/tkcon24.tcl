...
proc ::tkcon::Init {args} {
	...
	RCS		{RCS: @(#) $Id: tkcon24.tcl 1.14 2005/10/10 15:23:25Z rwon Exp $}
    ...
    set script_dir [file dirname [info script]]
    set PRIV(cmd_help_dir) [file join $script_dir ".." ".." ".." "doc" "contents.htm"]
	if {[string match windows $tcl_platform(platform)]} {
        set PRIV(edit_help_dir) [file join $script_dir ".." ".." ".." ".." "webhelp" "mergedProjects" "isptcl" "isptcl.htm#about_the_tcl_console.htm"]
        set PRIV(isp_help_dir) [file join $script_dir ".." ".." ".." ".." "webhelp" "main.htm"]
        set PRIV(opwebhlp) [file join $script_dir  ".." ".." ".." ".." "bin" "opwebhlp.exe"]

        bind all <F1> "callbrowser \"$::tkcon::PRIV(opwebhlp)\" \"$::tkcon::PRIV(edit_help_dir)\";break"
        #bind all <F1> "runCmd \"cmd.exe /C $::tkcon::PRIV(opwebhlp) $::tkcon::PRIV(edit_help_dir)\" 0;break"
    } else {
        set PRIV(edit_help_dir) [file join $script_dir ".." ".." ".." ".." ".." "webhelp" "mergedProjects" "isptcl" "isptcl.htm#about_the_tcl_console.htm"]
        set PRIV(isp_help_dir) [file join $script_dir ".." ".." ".." ".." ".." "webhelp" "main.htm"]
        bind all <F1> "runCmd \"opwebhlp $::tkcon::PRIV(edit_help_dir)\" 0;break"
    }
	...
	if {![info exists PRIV(histfile)]} {
	    ...
	    set PRIV(histfile)	[file join $dir $histfile]
	}
    }
    ...
    ## Load runcmd package
	EvalSlave package require "runcmd"
	## Turn off FOUNDRY popup msg boxes
	EvalSlave set env(NEO_NOMSGBOX) 1
...
proc ::tkcon::InitMenus {w title} {
    ...
    if {$tcl_platform(platform) == "windows"} {
        $m add command -label "Tcl Console Help" -command {callbrowser $::tkcon::PRIV(opwebhlp) $::tkcon::PRIV(edit_help_dir)}
        $m add command -label "Tcl/Tk Command Help" -command {callbrowser $::tkcon::PRIV(opwebhlp) $::tkcon::PRIV(cmd_help_dir)}
        $m add separator
        $m add command -label "ispLEVER Help" -command {callbrowser $::tkcon::PRIV(opwebhlp) $::tkcon::PRIV(isp_help_dir)}
    } else {
        $m add command -label "Tcl Console Help" -command {runCmd "opwebhlp $::tkcon::PRIV(edit_help_dir)" 0}
        $m add command -label "Tcl/Tk Command Help" -command {runCmd "opwebhlp $::tkcon::PRIV(cmd_help_dir)" 0}
        $m add separator
        $m add command -label "ispLEVER Help" -command {runCmd "opwebhlp $::tkcon::PRIV(isp_help_dir)" 0}
    }
