# These functions can be used in the tcl scripts of generate test fixture template
# Copyright (c), 2008-2009, Lattice Semiconductor Corporation, All Rights Reserved

package provide tbtemplate_generation 1.0.0
...
proc ::bali::GenerateTbTemplate {} {
	variable Para	
	
	#set command_line "hdl2jhd -tfi -mod $Para(MODNAME) -out $Para(OUTNAME) -tpl $Para(TFT) -ext $Para(EXT) -pf $Para(PRIMITIVEFILE) -pkg $Para(PACKAGE) -files"
	#foreach i $Para(FILELIST)	{
	#	lappend command_line $i
	#}
	set list_file [file join [pwd] $Para(MODNAME).lst] 
	GenerateFileList $list_file
	set command_line "hdl2jhd -tfi -proj $Para(PROJECT) -mod $Para(MODNAME) -out $Para(OUTNAME) -tpl $Para(TFT) -ext $Para(EXT) -pf $Para(PRIMITIVEFILE) -lst \"$list_file\""
